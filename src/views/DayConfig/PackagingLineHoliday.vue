  ...
  <div id="main-box">
    <el-card>
      <el-row>
        <el-col :span="16">
          <div>
            <el-button v-if="buttons.includes('PackagingLineHoliday/add')" type="primary" @click="addDataDialog">
              <i class="el-icon-plus" />添加
            </el-button>
            <el-button v-if="buttons.includes('PackagingLineHoliday/delete')" type="danger" @click="deleteData">
              <i class="el-icon-delete" />删除
            </el-button>
            <el-button v-if="buttons.includes('PackagingLineHoliday/import')" @click="importDataDialog">
    ...
    >
      ...
      <el-table
        ...
        border
      >
        <el-table-column prop="packaging_line_name" label="包装线名" />
        <el-table-column prop="holiday_days" label="放假天数" />
      </el-table>
      <el-row>
        <el-col :span="8">
...
export default {
  ...
  data() {
    return {
      ...
      tableDataExample: [
        {
          packaging_line_name: 'SM18',
          holiday_days: 200000.0
        }, {
          packaging_line_name: '(必填)',
          holiday_days: '(必填)'
        }
      ], // 示例的表格数据
  ...
  methods: {
    ...
    closeFormDialog() {
      ...
      this.model['packaging_line_name'] = ''
      this.modelOriginal['packaging_line_name'] = ''
      this.model['holiday_days'] = 0
      this.modelOriginal['holiday_days'] = 0
    ...
    handleDelete(index, row) {
      this.$confirm('确定要删除该数据？', '提示', {
        ...
        const data = {}
        data['id'] = row.id
        data['packaging_line_name'] = row.packaging_line_name
        data['user_name'] = this.name
