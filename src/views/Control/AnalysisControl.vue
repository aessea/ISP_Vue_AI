        ...
        <el-col :span="24">
          <el-tooltip class="item" effect="dark" :content="aiUnscheduledTip" placement="top">
            <el-button type="apiBtn" style="padding-left: 28px;padding-right: 28px" @click="post_ai_unscheduled">
              {{ $t('AnalysisControlPage.BtnPsuhAINot') }}
            </el-button>
          </el-tooltip>
...
import { SmtUnscheduled, SmtPrescheduled, SmtScheduled, AiUnscheduled, AiPrescheduled, AiScheduled } from '@/api/Control/DockingMes'
  ...
  methods: {
    ...
    loadExcelFile(file, fileList) {
      ...
      if (sessionStorage.getItem('lang') === 'zh') {
        ...
        regex = /^(0[1-9]|1[0-2])(0[1-9]|[12][0-9]|3[01])_(AIBoard|DJBoard)_(Preliminary|Regular).*$/
    ...
    post_ai_unscheduled() {
      ...
      AiUnscheduled(form).then(res => {
        if (res.code === 20000) {
          ...
          this.aiUnscheduledTip = this.$t('PublicText.MesApiPushed')
        } else {
          this.$alert(this.$t('PublicText.MesApiPushError'), this.$t('PublicText.TextError'), {
            confirmButtonText: this.$t('PublicBtn.Confirm'),
            type: 'error'
